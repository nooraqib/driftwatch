...
// A single line can match more than one pattern (e.g. the method-call
// pattern and a bare-word field pattern both matching "stripe.charges.
// create("). Keep one entry per file:line, preferring the longest/most
// specific matched symbol.
function dedupeByLine(sites) {
  const byLine = new Map();
  for (const site of sites) {
    const key = `${site.file}:${site.line}`;
    const existing = byLine.get(key);
    if (!existing || site.symbol.length > existing.symbol.length) {
      byLine.set(key, site);
    }
  }
  return Array.from(byLine.values()).sort((a, b) => a.file.localeCompare(b.file) || a.line - b.line);
}
